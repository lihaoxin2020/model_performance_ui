...
# KNOWLEDGE_SET="qwen-insturct-synthetic_1"
# KNOWLEDGE_SET="qwen-instruct-synthetic_1_stem_only"
# KNOWLEDGE_SET="qwen-instruct-synthetic_1_math_only"
# KNOWLEDGE_SET="llama3.1-instruct-synthetic_1"
# KNOWLEDGE_SET="llama3.1-instruct-synthetic_1_stem_only"
# KNOWLEDGE_SET="llama3.1-instruct-synthetic_1_math_only"

MODELS1=(
    # Models that DO use the assistant_prefix think tag
    "General-Reasoner-Qwen2.5-7B --num-shots 0 --model-args '{\"model_path\": \"TIGER-Lab/General-Reasoner-Qwen2.5-7B\", \"chat_model\": true, \"max_length\": 65536, \"rope_scaling\": {\"rope_type\": \"yarn\", \"factor\": 2, \"original_max_position_embeddings\": 32768}}'"
    ...
    # "Qwen2.5-7B-Instruct-${KNOWLEDGE_SET} --num-shots 0 --model-args '{\"model_path\": \"Qwen/Qwen2.5-7B-Instruct\", \"chat_model\": true, \"max_length\": 65536, \"rope_scaling\": {\"rope_type\": \"yarn\", \"factor\": 2, \"original_max_position_embeddings\": 32768}}'"
    ...
    # "Llama-3.1-8B-Instruct-${KNOWLEDGE_SET} --num-shots 0 --model-args '{\"model_path\": \"meta-llama/Llama-3.1-8B-Instruct\", \"tokenizer\": \"meta-llama/Llama-3.1-8B-Instruct\", \"chat_model\": true, \"max_length\": 65536}'"
    ...
    # "qwen3-8b-base-synthetic-sft-merged-no-thinking --num-shots 0 --model-args '{\"model_path\": \"lihaoxin2020/Qwen3-8B-Base-Synthetic-SFT-merged\", \"tokenizer\": \"Qwen/Qwen3-8B\", \"chat_model\": true, \"max_length\": 65536, \"rope_scaling\": {\"rope_type\": \"yarn\", \"factor\": 2, \"original_max_position_embeddings\": 32768}, \"enable_thinking\": false}'"
...
MODELS2=(
    ...
    # "qwen3-8b-base-synthetic-sft --num-shots 0 --model-args '{\"model_path\": \"beaker://haoxinl/qwen3-8b-base-synthetic-sft:model_cur\", \"tokenizer\": \"Qwen/Qwen3-8B\", \"chat_model\": true, \"max_length\": 65536, \"rope_scaling\": {\"rope_type\": \"yarn\", \"factor\": 2, \"original_max_position_embeddings\": 32768}}'"
    # "qwen3-8b-base-synthetic-sft-merged --num-shots 0 --model-args '{\"model_path\": \"lihaoxin2020/Qwen3-8B-Base-Synthetic-SFT-merged\", \"tokenizer\": \"Qwen/Qwen3-8B\", \"chat_model\": true, \"max_length\": 65536, \"rope_scaling\": {\"rope_type\": \"yarn\", \"factor\": 2, \"original_max_position_embeddings\": 32768}}'"
)
...
for MODEL in "${MODELS1[@]}"; do
    ...
    for TEMP in "${TEMPERATURES[@]}"; do
        ...
        MODEL_WITH_TEMP="${MODEL_BASE}-temp${TEMP} ${MODEL_REST}"
        ...
        if [ "$MODEL_TYPE" = "vllm-b200" ]; then
            ...
            --task-args '{\"chat_overrides\": {\"generation_kwargs\": {\"repeats\": ${REPEATS}, \"top_p\": ${TOP_P}, \"do_sample\": ${DO_SAMPLE},\"temperature\": ${TEMP}, \"max_gen_toks\": ${MAX_GEN_TOKS}, \"truncate_context\": false}}}'"
        else
            BASE_CMD="oe-eval --model ${MODEL_WITH_TEMP} --gpus ${GPUS} --num-workers ${NUM_WORKERS} --model-type vllm --batch-size ${BATCH_SIZE} --use-gantry \
            --gantry-args '${GANTRY_ARGS}' --beaker-image haoxinl/oe-eval-b200 --beaker-workspace ${WORKSPACE} --cluster ai2/jupiter-cirrascale-2,ai2/ceres-cirrascale,ai2/titan-cirrascale,ai2/augusta-google-1 \
            --task-args '{\"chat_overrides\": {\"generation_kwargs\": {\"repeats\": ${REPEATS}, \"top_p\": ${TOP_P}, \"do_sample\": ${DO_SAMPLE},\"temperature\": ${TEMP}, \"max_gen_toks\": ${MAX_GEN_TOKS}, \"truncate_context\": false}}}'"
        fi
    ...
    # && pip install vllm==0.8.5 && pip install transformers --upgrade
        ...
        for TASK in "${TASKS[@]}"; do
            ...
            if [[ -n "$EXPERIMENT_ID" ]]; then
                MODEL_NAME=$(echo "$MODEL_BASE" | cut -d' ' -f1)
                TASK_NAME=$(echo "$TASK" | cut -d' ' -f1)
                echo "${MODEL_NAME}-${TASK_NAME}:${EXPERIMENT_ID}" >> "$EXPERIMENT_LOG"
                echo "Logged experiment ID: ${MODEL_NAME}-${TASK_NAME}:${EXPERIMENT_ID}"
            fi
        done
